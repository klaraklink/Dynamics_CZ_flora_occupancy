...
occ.res.occ<-function(filepath){
  ...
  for (e in 1:length(RData.list)){
    ...
    jagsres.ci<-ci(jagsres.ggs) # calculate confidence intervals
...
plad.dc <- setDF(dcast(setDT(plad.raw), grid_year_auth+Nr_of_spec+half_dec+grid_small~name_lat, fun=length))
...
plad.raw <- read_csv("data_raw/pladias_occ_data_half_dec_20230621.csv") %>%
  select(name_lat, grid_small, half_dec) %>%
  distinct(name_lat, grid_small, half_dec) 

# clipdf for clipping results
clip.df <- plad.raw %>% 
  select(name_lat, half_dec) %>% 
  group_by(name_lat) %>% 
  summarize(clip.min = min(half_dec), clip.max = max(half_dec))

write_csv(clip.df, "results/clip_df.csv")
