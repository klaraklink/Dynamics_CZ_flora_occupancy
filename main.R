...
#' 
#' Author: Klára Klinkovská, Michael Glaser, 2023-09-22
#' R version 4.3.0
...
# plot settings -----------------------------------------------------------
path.plots <- "plots_mod_ht/"

wc.col1<-substr(rgb(84,130,53,1,maxColorValue=255),1,7)
wc.col2<-substr(rgb(169,209,142,1,maxColorValue=256),1,7)
wc.grey<-substr(rgb(166,166,166,1,maxColorValue=256),1,7)
...
### Mytheme
txt.tit<-unit(14,"points")
txt.l<-unit(12,"points")
txt.m<-unit(10,"points")
txt.s<-unit(8,"points")
t.size<-3
t.dist<-20

my.theme<-theme(
  plot.title=element_text(size=txt.tit, face="bold"),
  axis.title=element_text(size=txt.m, face="bold"),
  axis.text=element_text(size=txt.s), 
  legend.title=element_text(size=txt.m, face="bold"), 
  legend.text=element_text(size=txt.s))
...
chi.test <- function(col1, col2, exclude, excluded, sim) {
  cont.tab <- resdf.clip.join %>% 
    ...
    group_by(change.lm) %>% 
    dplyr::summarize_at(vars(col1:col2), sum, na.rm = T) %>% 
    filter(change.lm != 0) %>% 
...
### test for differences between groups created by the time-series clustering
...
test1 <- read_csv("results/occ_results_occupancy_60half_20230801.csv") %>% 
...
resdf.clip <- test1 %>% 
  left_join(read_csv("results/records_decades.csv") %>% 
              filter(pres != 0) %>% 
              group_by(name_lat) %>% 
              dplyr::summarize(clip.min = min(half_dec), clip.max = max(half_dec)), by = c("specname" = "name_lat")) %>% 
  ...
  dplyr::select(specname, lm.slope, lm.p) %>%
  ungroup() %>% 
...
resdf.clip.join <- resdf.clip %>% 
  ...
  mutate(alien = revalue(`Residence time`, replace=c("ar NE" = "archaeophyte",
         ...
         IUCN = str_replace(IUCN, "LC\\(NA\\)|NA", "LC") %>% replace_na("LC"), 
  ...
  dplyr::rename("Par_parasite" = "Par_root_hemiparasite", "Par_mycoheterotroph" = "Par_full_mycoheterotroph", 
  ...
  dplyr::select(-c(GR_fac_apomixis, GR_obl_apomixis, GR_fac_autogamy, GR_fac_allogamy, 
...
# filter only top/bottom 50 species and select important columns
tb50.res <- resdf.clip.join %>% 
  filter(lm.rank %in% c(1:50, (nrow(resdf.clip.join)-49):nrow(resdf.clip.join))) %>% 
  select(lm.rank, specname, lm.slope, IUCN, alien) %>% 
  mutate(lm.rank = rank(lm.slope), IUCN = fct_relevel(IUCN, c("CR", "EN", "VU", "NT", "LC")), 
...
ggplot(tb50.res)+
  ...
  geom_text(aes(color = alien, x=lm.rank, y=just, label=specname), 
            hjust = c(rep(0, nrow(tb50.res)/2), rep(1, nrow(tb50.res)/2)),
            size=2.1)+
  scale_fill_manual(values = iucn.col)+
  scale_color_manual(values = c("native" = "black", "archaeophyte" = "#3f37c9", "neophyte" = "#b5179e"))+
  coord_flip()+
...
ggsave(paste0(path.plots,"tb50.png"), width=10, height = 24, units=c("cm"))
...
# Appendix S5 correlation of variables in model
...
corrplot.all <- corrplot(cor.all, method="number", tl.cex=1, number.cex=0.7, 
...
# save to svg
svglite(paste0(path.plots,"linear_models.svg"), width = 11, height = 8)
plot.lm
dev.off()
...
full.mod <- lm(lm.slope~EIV_moist+EIV_nutr+dist_freq+
                dist_sev+eco_spec_all+colonization_success+
                LS_Pierce_S+LS_Pierce_C, data = model.df)
...
comp5.cvi <- comp5$objects.partitional %>% map(~cvi(.x))

comp5.cvi2 <- comp5.cvi %>% 
  unlist() %>% 
  as.data.frame() %>% 
...
spe.clust <- resdf.clip.join2 %>% 
  select(specname, cluster)

write_csv(spe.clust, "clustering_results.csv")
...
# replace anova and tukey results with Kruskal-Wallis
...
# differences in categorical traits between clusters ----------------------
...
# export table with species trends Appendix S7 ------------------------
spe.clust <- read_csv("clustering_results.csv")

resdf.clip2 <- test1 %>% 
  left_join(read_csv("results/records_decades.csv") %>% 
              filter(pres != 0) %>% 
              group_by(specname) %>% 
              dplyr::summarize(clip.min = min(half_dec), clip.max = max(half_dec))) %>% 
  filter(parnum >= clip.min & parnum <= clip.max & !specname %in% n.conv$specname 
         & !specname %in% n.conv2$specname) %>% 
  group_by(specname) %>% 
  nest() %>%
...
resdf.clip.join.pub <- left_join(resdf.clip2, spe.clust) %>% 
